...
## � 自动联网情报(状态/伤停/动机)
每次更新会自动为每场比赛联网检索情报, 不需手工录入:
- 检索: 「头条搜索」公开摘要(`so.toutiao.com`), 关键词 = 联赛 + 主队 + 客队 + 伤停/首发/前瞻
- 归纳: DeepSeek 把摘要压成一句话(近期状态 / 伤停缺阵 / 动机战意) + 风险档 + 微调值 adj(±1 内)
- 缓存: `data/web_intel.json`(按销售日, 保留最近 7 天), 同一天重复构建网页不会重复请求
- 用途: ① 网页「🎯 串关方案 → 📰 赛前情报」展示; ② **做胆核验**——情报不足或显示风险高 → 降级「🟡 观望」
- 局限: 搜索摘要可能过期或与比赛无关, 仅供参考; 想覆盖时仍可手写 `data/extra_intel.json`

## 📝 手动情报(伤病/转会/教练/战意)
有把握的情报可手写 `data/extra_intel.json`（`adj` -3~+3，正=利好主队），保存后下次更新生效；
手写情报优先于自动联网情报。
