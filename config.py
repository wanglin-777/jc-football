...
# 建议1(严格胆材): 只有 预测胜率>=BANKER_MIN_PROB 且 赔率<=BANKER_MAX_ODDS 才列为单关胆材
#   且必须通过"近期状态(完整情报) + 伤停/动机核验(手动或自动联网情报)"; 未通过 -> 降级为"观望"
# 2026-09-16 按 AI 复盘调整: 60-70% 档实际只中 54%(偏虚高), 且 40-50% 档实际 36% 明显虚高
#   -> 做胆门槛由 0.60 上调至 0.65; 40-50% 档不再参与推荐(所有推荐下限均为 MIN_PROB_LEG)
BANKER_MIN_PROB = 0.65
BANKER_MAX_ODDS = 1.55
BANKER_REQUIRE_VERIFY = True      # 做胆需状态+情报核验
# 建议2(串关避险): 串关选腿优先"低爆冷风险 + 胜率差>=COMBO_MARGIN_TIERS[0]"
#   并额外剔除: 强队主场 + 赔率偏低 + 对手有保级/反弹动机 的场次(见下方阈值)
COMBO_MARGIN_TIERS = (0.25, 0.18, 0.10)   # 低风险档的胜率差门槛
MIN_PROB_LEG = 0.50                        # 串关单腿的最低模型胜率(稳)
AVOID_SHORT_ODDS_MAX = 1.60    # "赔率偏低"阈值(强队低赔)
AVOID_AWAY_RANK_MIN = 14       # 客队排名>=此值视为保级区/低位(有反弹动机)
AVOID_AWAY_FORM_MAX = 0.20     # 客队胜率<=此值视为状态差(有反弹动机)
# 建议3(补平局): 两队近期平局率均不低于此值时, 模型强制给"平局"加权
DRAW_PRONE_RATE = 0.25
# 2026-09-16 AI 复盘: 实际平局 42 场, 模型只主动预测 2 场(漏判 98%)
#   -> 先不动 pick, 但把"势均力敌 + 平赔偏低"的场次标为「平局候选(防平)」并单独统计命中率,
#      样本足够后再决定是否真的把平作为首选(避免盲目强制选平拖低总体命中率)。
DRAW_CAND_ODDS_MAX = 3.00      # 平赔 <= 此值才考虑
DRAW_CAND_MIN_P = 0.28         # 模型平局概率 >= 此值
DRAW_CAND_SIDE_GAP = 0.12      # 主/客胜概率差 <= 此值(势均力敌)
