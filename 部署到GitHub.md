...
| Actions 失败 | 点进红色任务看日志。默认从 **500彩票网** 抓取(海外可访问)；官方体彩官网封锁境外 IP(HTTP 567) 仅作中国网络内备选。若 500 也失败会**保留上次成功的数据**不覆盖 |
| 手机打不开 | 确认网址拼写；GitHub Pages 是公开网站，无需任何额外操作 |
| 想改更新间隔 | 编辑 `.github/workflows/update.yml` 里的 `cron`，比如每 1 小时：`cron: '17 * * * *'` |
| 完全云端? | 是。数据抓取/建模/更新全在 GitHub Actions 上运行，**无需你的电脑开机** |
...
> 说明：网页内容 = 竞彩场次/赔率(官方或500网) + 泊松模型预测（详见 README）。
> 伤病/转会/战意等人工情报字段在云端任务里默认不填；若想启用，把填写好的
> `data/extra_intel.json` 一起提交进仓库即可（该文件默认未忽略）。
>
> 数据源选择逻辑：云端运行 `build_site.py` → 先尝试体彩官方接口(境外会被 567 拦截)
> → 自动改从 **500彩票网** 抓取当天竞彩(同官方赔率口径) → 仍失败才用缓存。
